...
if MyAccount then
    ...
    function checkgatcan()
        local a =
            game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CheckTempleDoor")
        if a then
            return true
        else
            return false
    ...
    local function v5()
        ...
        for v6, v7 in pairs(
            {"Last Resort", "Agility", "Water Body", "Heavenly Blood", "Heightened Senses", "Energy Core"}
        ) do
    ...
    function checkrace()
        ...
        if
            not game:GetService("Players").LocalPlayer.Data.Race:FindFirstChild("Evolved") and not v5() and
                not game.Players.LocalPlayer.Character:FindFirstChild("RaceTransformed")
         then
